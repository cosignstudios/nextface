...
const Login = () => {
  ...
  const googleLogin = useGoogleLogin({
    ...
    onError: () => setError("Google login interrupted"),
  });
      ...
      <div className="w-full max-w-md card-brutal bg-[var(--surface-white)] !p-10">
        {view === "email" && (
          <button 
            ...
            className="mb-6 flex items-center gap-2 text-xs font-black uppercase hover:text-brutal-blue transition-colors group"
          >
        ...
        <h2 className="text-3xl font-black text-[var(--text-color)] mb-8 text-center uppercase tracking-tighter italic">
          {view === "selection" ? "Entry" : "Standard"} <span className="bg-brutal-blue px-2 border-2 border-black inline-block transform -rotate-1 text-black">Protocol</span>
        </h2>
        ...
        {view === "selection" ? (
          <div className="space-y-6">
            ...
            >
              ...
              <span className="text-sm font-black uppercase tracking-widest">Login with Google</span>
            </button>

            <button
              onClick={() => setView("email")}
              className="w-full btn-brutal bg-brutal-blue text-sm font-black uppercase tracking-widest py-4"
            >
              Login with Email
            </button>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full btn-brutal bg-brutal-blue text-lg"
            >
              Enter Protocol
            </button>
          </form>
        ...
        <div className="mt-10 pt-6 border-t-2 border-black text-center">
          <p className="font-bold uppercase text-[10px] tracking-tight text-gray-500">
            {view === "selection" ? "No account recorded?" : "Verification needed?"}{" "}
            <Link to="/register" className="underline decoration-brutal-pink decoration-2 underline-offset-4 hover:bg-brutal-pink px-1">
              Generate Identity
            </Link>
          </p>
        </div>
