      ...
      <main className="flex flex-col lg:flex-row flex-1 min-h-0 w-full overflow-hidden p-2 lg:p-6 gap-2 lg:gap-6">
        ...
        <aside className="w-full lg:w-80 xl:w-96 shrink-0 h-[40vh] lg:h-full overflow-hidden">
          <div className="h-full card-brutal !p-0 bg-black relative flex flex-col group overflow-hidden">
            ...
            {/* Floating Top Label */}
            <div className="absolute top-4 right-4 z-10 px-3 py-1 bg-black/50 backdrop-blur-md border border-white/20 text-[10px] font-black uppercase tracking-widest text-white transition-all duration-300">
              Local Port: Secured
            </div>

            {/* Minimalist Message History Overlay */}
            <div className="absolute inset-x-4 bottom-20 z-20 flex flex-col gap-2 min-h-0 max-h-[60%] pointer-events-none">
            ...
            {/* Floating Input Area */}
            <div className="absolute inset-x-4 bottom-4 z-20 flex gap-2 bg-black/40 backdrop-blur-2xl p-2 border border-white/10 rounded-xl shadow-2xl">
                <input
                  type="text"
                  value={chatMessage}
